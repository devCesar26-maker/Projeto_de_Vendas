...
# =====================================================================
# CONFIGURAÇÃO DE DESIGN E TEMA RESPONSIVO EM CORES CLARAS
# =====================================================================
st.set_page_config(
...
# Injeção de CSS focada em Flexbox e Layouts 100% Fluídos
st.markdown("""
    <style>
    .stApp {
    ...
    /* Forçar os contêineres dos gráficos a aceitarem 100% de largura dinamicamente */
...
st.title("Painel Executivo de Performance")
st.markdown("---")


# =====================================================================
# 1. CARREGAMENTO DOS DADOS (ÚNICA FONTE)
# =====================================================================
@st.cache_data
def carregar_dados_base(base):
    return pd.read_csv(base)

# Lemos tudo diretamente da base limpa unificada
...
# =====================================================================
st.sidebar.markdown("Filtros do Panel")
...
# Agrupamento Mensal (Para o novo gráfico de linha inferior)
...
# Agrupamento Semanal (Para o novo gráfico de barras inferior)
ordem_dias = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado', 'Domingo']

analise_semanal = df_filtrado.groupby("Dia_Semana", observed=False)["Faturamento"].sum().reset_index()

# Força o DataFrame a organizar os dias na ordem correta da semana, em vez da ordem alfabética
analise_semanal['Dia_Semana'] = pd.Categorical(analise_semanal['Dia_Semana'], categories=ordem_dias, ordered=True)
analise_semanal = analise_semanal.sort_values(by="Dia_Semana").reset_index(drop=True)



# =====================================================================
# 5. SEÇÃO DE GRÁFICOS INTERATIVOS 100% RESPONSIVOS
# =====================================================================
col_grafico_esq, col_grafico_dir = st.columns([1, 1])

# --- GRÁFICO 1 (Esquerda): Curva de Pareto ---
with col_grafico_esq:
    st.subheader("Curva de Pareto por Produto")
    ...
    fig_pareto.update_layout(
        template="plotly_white", showlegend=False, autosize=True,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#ffffff", margin=dict(l=40, r=40, t=10, b=40),
        yaxis=dict(title="Faturamento (R$)", title_font=dict(size=11, color=COR_TEXTO_EIXO, weight="bold"), tickformat="$,.0f", gridcolor=COR_GRID, tickfont=dict(color=COR_TEXTO_EIXO)),
        yaxis2=dict(title="% Acumulado", title_font=dict(size=11, color=COR_TEXTO_EIXO, weight="bold"), overlaying="y", side="right", range=[0, 105], ticksuffix="%", showgrid=False, tickfont=dict(color=COR_TEXTO_EIXO))
    )
    fig_pareto.update_xaxes(tickangle=15, tickfont=dict(size=10, color=COR_TEXTO_EIXO))
    st.plotly_chart(fig_pareto, use_container_width=True)


# --- GRÁFICO 2 (Direita): Ranking de Filiais ---
with col_grafico_dir:
    st.subheader("Ranking de Filiais (Rótulos: TM e Pedidos)")
    ...
    fig_lojas.add_trace(go.Bar(
        ...
        # Atualizado: O hover agora também mostra a quantidade de pedidos ao passar o mouse
        hovertemplate="<b>Filial %{y}</b><br>Faturamento: R$ %{x:,.2f}<extra></extra>"
    ))
    
    anotacoes = []
    max_x = analise_filial["Faturamento_Regional"].max() if not analise_filial.empty else 0
    
    for index, row in analise_filial.iterrows():
        # Formatações brasileiras para moeda e número inteiro
        ...
        # Concatenamos o Ticket Médio com o número de pedidos em uma string elegante
        texto_rotulo = f"<b>{tm_formatado} | Vol: {pedidos_num_formatado}</b>"
        
        anotacoes.append(dict(
            x=row["Faturamento_Regional"], y=row["Loja"],
            text=texto_rotulo, xanchor='left', yanchor='middle',
            showarrow=False, font=dict(size=12, color=COR_TEXTO_TM), xshift=8
        ))
        
    fig_lojas.update_layout(
        template="plotly_white", showlegend=False, annotations=anotacoes, autosize=True,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="#ffffff", margin=dict(l=90, r=160, t=10, b=40), # Aumentei a margem direita (r=160) para o texto não cortar
        xaxis=dict(
            title="Faturamento Regional (R$)", title_font=dict(size=11, color=COR_TEXTO_EIXO, weight="bold"),
            tickformat="$,.0f", gridcolor=COR_GRID, tickfont=dict(color=COR_TEXTO_EIXO),
            range=[0, max_x * 1.55] # Expandido dinamicamente para acomodar o texto maior sem sobrepor
        ),
        yaxis=dict(tickfont=dict(size=11, color=COR_TEXTO_EIXO, weight="bold"))
    )
    st.plotly_chart(fig_lojas, use_container_width=True)

# --- GRÁFICO 3 (Inferior): Faturamento Mensal Expandido ---
st.markdown("---")
...
fig_mensal.add_trace(go.Scatter(
    ...
    line=dict(color=COR_BARRA_PROD, width=4, shape="spline"), # Linha suavizada moderna
...
fig_mensal.update_layout(
    ...
    showlegend=False,
    autosize=True,
    height=300, # Uma altura confortável para o gráfico de linha horizontal expandido
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="#ffffff",
    margin=dict(l=60, r=40, t=10, b=40),
    xaxis=dict(
        title="Período (Mês-Ano)",
        title_font=dict(size=11, color=COR_TEXTO_EIXO, weight="bold"),
...
# =====================================================================
# --- GRÁFICO 4 (Inferior): Faturamento por Dia da Semana (Abordagem Barras Horizontais) ---
# =====================================================================
st.markdown("---")
st.subheader("Análise Operacional: Faturamento por Dia da Semana")

# 1. Agrupamento limpo e direto por dia da semana
ordem_dias = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado', 'Domingo']

analise_semanal = df_filtrado.groupby("Dia_Semana", observed=False)["Faturamento"].sum().reset_index()
analise_semanal['Dia_Semana'] = pd.Categorical(analise_semanal['Dia_Semana'], categories=ordem_dias, ordered=True)

# Invertemos a ordem aqui para que a Segunda-feira apareça no topo do gráfico e o Domingo na base
analise_semanal = analise_semanal.sort_values(by="Dia_Semana", ascending=False).reset_index(drop=True)

fig_semana = go.Figure()

# Criamos as barras horizontais (Usando orientation='h')
fig_semana.add_trace(go.Bar(
    ...
    name="Faturamento",
    marker=dict(
        color=analise_semanal["Faturamento"],
        colorscale="Blues", # Lindo degradê corporativo
        line=dict(width=0)
    ),
...
fig_semana.update_layout(
    ...
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="#ffffff",
    margin=dict(l=110, r=50, t=10, b=40),
    xaxis=dict(
    ...
    yaxis=dict(
        tickfont=dict(color=COR_TEXTO_EIXO, size=11, weight="bold"),
        gridcolor="rgba(0,0,0,0)" # Limpa linhas horizontais desnecessárias
    )
)
